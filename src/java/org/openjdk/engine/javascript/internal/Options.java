...
final class Options {
    ...
    private static final String NASHORN_ARGS_PREPEND_PROPERTY = "jvmv8.args.prepend";

    /** System property that can be used to append options to the explicitly specified command line. */
    private static final String NASHORN_ARGS_PROPERTY = "jvmv8.args";
     ...
     *
     * @param resource resource prefix for options e.g. "jvmv8"
     */
    ...
    /**
     * Get the resource for this Options set, e.g. "jvmv8"
     * @return the resource
     */
    ...
    private static void checkPropertyName(final String name) {
        if (! Objects.requireNonNull(name).startsWith("jvmv8.")) {
     ...
     * @param shortKey the short (that is without the e.g. "jvmv8.option." part) key
     ...
     */
    private static final String MESSAGES_RESOURCE = "org.openjdk.engine.javascript.internal.resources.Options";
    ...
    static {
        ...
        for (final Enumeration<String> keys = Options.bundle.getKeys(); keys.hasMoreElements(); ) {
            ...
            if (st.countTokens() > 0) {
                resource = st.nextToken(); // e.g. "jvmv8"
            }
