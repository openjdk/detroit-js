...
/**
 * This is the main class for JavaScript shell.
 */
final class Main {
    private Main() {}
    ...
    private static void addStringSource(final ScriptEngine se,
        ...
        scripts.add(args -> {
            setScriptArguments(se, args);
            Object oldFile = setScriptFilename(se, "<string>");
            try {
    ...
    private static void listScriptEngines() {
        ...
        for (ScriptEngineFactory factory: factories) {
            getError().println(V8.getMessage("engine.info",
    ...
    private static void processSource(ScriptEngine se, String filename,
        ...
        if (filename.equals("-")) {
            ...
            while (true) {
                ...
                if (source == null || "quit".equals(source)) {
                    break;
                }
                Object res = evaluateString(se, source, false);
                if (res != V8Undefined.INSTANCE) {
                    try {
                        res = res == null ? "null" : res.toString();
                        getError().println(res);
                    } catch (Exception exp) {
                        getError().println(exp.getMessage());
                    }
                }
            }
        } else {
            ...
            try {
                ...
                getError().println(V8.getMessage("file.not.found",
                        new Object[] { filename }));
    ...
    private static Object evaluateString(ScriptEngine se,
        ...
        try {
            ...
            getError().println(V8.getMessage("string.script.error",
    ...
    private static Object evaluateReader(ScriptEngine se,
        ...
        Object oldFilename = setScriptFilename(se, name);
        try {
            return se.eval(reader);
        } catch (ScriptException sexp) {
            getError().println(V8.getMessage("file.script.error",
    ...
    private static Object evaluateStream(ScriptEngine se,
        ...
        if (encoding != null) {
            try {
                ...
                getError().println(V8.getMessage("encoding.unsupported",
                        new Object[] { encoding }));
    ...
    private static void usage(int exitCode) {
        getError().println(V8.getMessage("main.usage",
    ...
    private static ScriptEngine getScriptEngine(String lang) {
        ...
        if (se == null) {
            ...
            if (se == null) {
                getError().println(V8.getMessage("engine.not.found",
                        new Object[] { lang }));
                        System.exit(EXIT_ENGINE_NOT_FOUND);
            }

            // initialize the engine
            initScriptEngine(se, "v8".equals(lang.toLowerCase()));
    ...
    private static void initScriptEngine(ScriptEngine se, boolean v8) {
    ...
    private static URL fileToURL(File file) {
        ...
        try {
            return new URI("file", name, "").toURL();
    ...
    private static Object setScriptFilename(ScriptEngine se, Object name) {
        Object oldName = se.get(ScriptEngine.FILENAME);
    ...
    private static final String DEFAULT_LANGUAGE = "v8";
    // list of scripts to process
    private static final List<Command> scripts = new ArrayList<>();
    // the script engine manager
    private static ScriptEngineManager engineManager;
    // map of engines we loaded
    private static final Map<String, ScriptEngine> engines = new HashMap<>();
    // program name
    private static final String PROGRAM_NAME = "jjs";
}
