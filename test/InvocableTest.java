...
/**
 * Tests for javax.script.Invocable implementation of v8.
 *
 * @test
 * @build VariableArityTestInterface InvocableTest
 * @run testng InvocableTest
 */
...
public class InvocableTest {
    ...
    public void invokeMethodMixEnginesTest() {
        final ScriptEngineManager m = new ScriptEngineManager();
        final ScriptEngine engine1 = m.getEngineByName(ENGINE);
    ...
    public void getInterfaceMissingTest() {
        ...
        final ScriptEngine engine = manager.getEngineByName(ENGINE);
        ...
        // should not throw NPE now!
        runnable.run();

        // define only one method of "Foo2"
        try {
            engine.eval("function bar() { }");
        } catch (final Exception exp) {
            exp.printStackTrace();
            fail(exp.getMessage());
        }
    ...
    @Test
    public void scriptInterfaceImplTest() throws ScriptException {
        final ScriptEngineManager m = new ScriptEngineManager();
        final ScriptEngine e = m.getEngineByName("v8");
        e.eval("var R = java.lang.Runnable;");
        e.eval("r = new R({ run: function() { print('runnable') } })");
        assertTrue((Boolean)e.eval("r instanceof R"));
        e.eval("r.run()");
    }

    @Test
    public void scriptInterfaceImplFailureTest() throws ScriptException {
        final ScriptEngineManager m = new ScriptEngineManager();
        final ScriptEngine e = m.getEngineByName("v8");
        e.eval("var R = java.lang.Runnable;");
        ...
        try {
            ...
            assertEquals(((JSObject)se.getEcmaError()).getMember("name"), "TypeError");
    ...
    public void variableArityInterfaceTest() throws ScriptException {
        final ScriptEngineManager m = new ScriptEngineManager();
        final ScriptEngine e = m.getEngineByName("v8"); // requires Java access
        e.eval(
